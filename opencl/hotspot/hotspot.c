...
int main(int argc, char **argv) {
    ...
    cl_float *MatrixOut =
	(cl_float *) clEnqueueMapBuffer(command_queue, MatrixTemp[ret], CL_TRUE,
					CL_MAP_READ, 0, sizeof(float) * size, 0,
					NULL, NULL, &error);
    if (error != CL_SUCCESS)
	fatal_CL(error, __LINE__);
    ...
    error =
	clEnqueueUnmapMemObject(command_queue, MatrixTemp[ret],
				(void *)MatrixOut, 0, NULL, NULL);
    if (error != CL_SUCCESS)
	fatal_CL(error, __LINE__);
